...
`index.html` loads the `.jsx` files as plain scripts in a fixed order — **there are no ES module imports/exports**. Files communicate through globals on `window`. Order: `tweaks-panel.jsx` → `store.jsx` → `components.jsx` → `academic.jsx` → `research.jsx` → `finance.jsx` → `travel.jsx` → `app.jsx` (App must be last). A file can only reference globals defined by files loaded before it. When adding a file, add a `<script>` tag in the correct position.
...
- **`components.jsx`** — shared/household-ish panels (`ProjectPanel`, `ReadingPanel`, `ShoppingPanel`, `HabitPanel`, `ChoresPanel`, `ReminderPanel`, `GoalsPanel`) plus primitives like `AutoTextarea`, `EditButton`, and **`MetaRow`** — the one shared *tagged dated row* (a `lead` checkbox/status-pill + chip + title + `.meta-row-when` date + `.meta-row-value` money + edit/delete, every slot optional). It is the single superset behind Deadlines, Bills, Reimbursements, Documents, and the research Submissions/Proposals/CFP/Contacts rows; it replaced the old per-panel `.deadline-row`/`.bill-item`/`.reimb-item`/`.doc-item` namespaces (see DESIGN.md §3 "Tagged dated row"). Most panels are `memo`'d and follow an `{ items, onChange }` prop contract. On the finance and travel tabs the aspirational `GoalsPanel` sits at the top of the right-hand column (top-right).
